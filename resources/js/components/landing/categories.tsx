...
import { Palette, PenTool, Video, Globe, Layout, Brush, ArrowRight, type LucideIcon } from "lucide-react"
...
const categoryMeta: Record<string, { description: string; icon: LucideIcon; color: string }> = {
  "Graphic Designer": {
...
export function Categories({ categoryCounts = [] }: Props) {
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {categoryCounts.map((category) => {
            ...
            return (
              <Link href={`/explore?category=${category.slug}`} key={category.name}>
                <Card className="group cursor-pointer border-border/50 hover:border-primary/50 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300">
                  <CardContent className="p-6">
                    <div className="flex items-start gap-4">
                      <div className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${meta.color} shadow-lg`}>
